...
module.exports = {
  Organization: {
    ...
    members: async (obj, args, context) => {
      ...
      const docs = await context.loaders.users
  ...
  Mutation: {
    createOrganization: async (obj, args, context) => {
      ...
      const doc = await context.loaders.organizations.insertOne({
        _id: uuid(),
        createdAt: isoDate,
      ...
      return context.loaders.organizations.findOneCachedById(doc.ops[0]._id);
    },
    setMemberAsAdmin: async (obj, args, context) => {
      const isoDate = new Date().toISOString();
      await context.loaders.organizations.updateOne(
        { _id: args.organizationId, 'users.id': args.memberId },
        { $set: { lastModifiedAt: isoDate, 'users.$.isAdmin': true } }
      );
      return context.loaders.organizations.findOneCachedById(
        args.organizationId
      );
    },
